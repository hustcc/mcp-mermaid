...
- Support exporting to `base64`, `svg`, `mermaid`, `png_file`, `svg_file`, and remote-friendly `svg_url`, `png_url` formats, with validation for `Mermaid` to facilitate the model's multi-round output of correct syntax and graphics. Use `outputType: "png_file"` to automatically save PNG diagrams to disk for AI agents, `outputType: "svg_file"` for SVG diagrams, or the URL modes to share diagrams through public mermaid.ink links.
